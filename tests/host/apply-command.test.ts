...
function revisionOf(fs: FakeFs): string {
  ...
  return sha256(sorted.map(([path, content]) => `${path}\n${sha256(content)}\n`).join(''))
}
...
describe('backStep and forwardStep', () => {
  ...
  async function setupWithUndo(record: Record<string, unknown>) {
    const { fs, git, storage, host } = await setup({ '/proj/research.json': MANIFEST, [`/proj/nodes/${NODE}.md`]: nodeFile })
    const undoTable = storage.tableOf(UI_STATE_DOMAIN, TABLE_UNDO)!
